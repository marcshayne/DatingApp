...
namespace DatingApp.API.Data
{
    public class DatingRepository : IDatingRepository
    {
        ...
        public async Task<PagedList<Message>> GetMessagesForUser(MessageParams messageParams) //L.163
        {
            var messages = _context.Messages.Include(u =>u.Sender)
                .ThenInclude(p => p.Photos) //to display the user's main photo
                .Include(u => u.Recipient)
                .ThenInclude(p => p.Photos)
                .AsQueryable();  // because we are adding a where clause
